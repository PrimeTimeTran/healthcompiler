        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-primary/20 bg-primary/5">
              ...
              <span className="text-sm font-medium text-primary">Risk Adjustment Intelligence</span>
            </div>
            ...
            <p className="text-lg md:text-xl text-muted-foreground leading-relaxed max-w-xl">
              Proactively identify likely but undocumented chronic conditions that matter for care quality and accurate reimbursement.
            </p>

            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="group" asChild>
                <Link to="/contact">
                  Explore HCC Suspecting
              ...
              {[0, 1, 2, 3].map((i) => (
                <g key={i}>
                  <path
                    ...
                    strokeWidth="2"
                    strokeDasharray="8 4"
                    className={`transition-opacity duration-500 ${i === activeSource ? 'opacity-100' : 'opacity-20'}`}
                  />
                  {i === activeSource && (
...
const WhyItMattersSection = () => {
  const outcomes = [
    { icon: FileText, title: 'Close Documentation Gaps', desc: 'Find conditions not yet coded' },
    { icon: TrendingUp, title: 'Boost RAF Accuracy', desc: 'Improve risk score precision' },
    { icon: Heart, title: 'Drive Better Care Decisions', desc: 'Clinicians see the right picture' },
  ];
...
const HowItWorksSection = () => {
  ...
  const steps = [
    { title: 'Aggregate', desc: 'Unify claims, EHR, labs, Rx, and notes', icon: '📊' },
    { title: 'Detect', desc: 'Surface patterns and likely conditions', icon: '🔍' },
    { title: 'Suggest', desc: 'Present suspect HCCs with clinical evidence', icon: '💡' },
    ...
    <section ref={sectionRef} className="py-24 bg-gradient-to-b from-slate-50 to-white">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground">How HCC Suspecting transforms data into actionable insights</p>
        </div>
          ...
          <div className="grid md:grid-cols-4 gap-8 relative">
            {steps.map((step, idx) => (
              ...
              >
                ...
                >
                  <span className={idx <= activeStep ? 'grayscale-0' : 'grayscale'}>{step.icon}</span>
                </div>
...
const ResultsSection = () => {
  const results = [
    { before: 'Incomplete risk capture', after: 'Complete HCC-level picture', icon: Target },
    { before: 'Low RAF scores', after: 'Aligned reimbursement', icon: TrendingUp },
    { before: 'Reactive coding reviews', after: 'Proactive suspecting insights', icon: Zap },
  ];
...
const BuiltForHealthcareSection = () => {
  const features = [
    { icon: Activity, text: 'Integrates with EHR, claims, labs, and Rx at scale' },
    { icon: Clock, text: 'Real-time insights at point of care and point of coding' },
    { icon: Shield, text: 'Designed for Medicare Advantage, ACOs, and value-based models' },
  ];
