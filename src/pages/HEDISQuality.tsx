...
// ========== HERO SECTION WITH QUALITY GRID ANIMATION ==========
const HeroSection = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMeasures(prev => prev.map((measure, idx) => ({
        ...measure,
  ...
  const getStatusColor = (status: number) => {
    if (status === 0) return { bg: 'bg-red-100', border: 'border-red-300', dot: 'bg-red-500' };
    if (status === 1) return { bg: 'bg-yellow-100', border: 'border-yellow-300', dot: 'bg-yellow-500' };
    return { bg: 'bg-green-100', border: 'border-green-300', dot: 'bg-green-500' };
  };
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-primary/20 bg-primary/5">
              ...
              <span className="text-sm font-medium text-primary">HEDIS & Quality Measures</span>
            </div>
            ...
            <p className="text-lg md:text-xl text-muted-foreground leading-relaxed max-w-xl">
              Track, close, and improve HEDIS and quality gaps in real time—without chasing reports or year-end fire drills.
            </p>

            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="group" asChild>
                <Link to="/contact">
                  Explore Quality Measures
            ...
            <div className="relative w-full max-w-sm">
              ...
              <div className="bg-white rounded-t-2xl border border-b-0 border-border p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-foreground">Quality Dashboard</h3>
                    <p className="text-xs text-muted-foreground">Real-time measure tracking</p>
                  </div>
                  ...
                  {measures.map((measure, idx) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          animation: measure.status === 2 ? 'pulse 2s ease-in-out infinite' : 'none',
                        }}
                      >
                        ...
                        <div className="text-lg font-bold text-foreground">
                          {measure.status === 0 ? '—' : measure.status === 1 ? '~' : '✓'}
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  <div className="flex items-center justify-between text-xs text-muted-foreground mb-2">
                    ...
                    <span className="font-semibold text-foreground">
                      {Math.round((measures.filter(m => m.status === 2).length / measures.length) * 100)}%
                    </span>
                  </div>
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-gradient-to-r from-primary to-accent transition-all duration-700 rounded-full"
                      style={{ width: `${(measures.filter(m => m.status === 2).length / measures.length) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
...
const WhyItMattersSection = () => {
  const outcomes = [
    { icon: Target, title: 'Close Care Gaps', desc: 'Identify patients who need action now' },
    { icon: TrendingUp, title: 'Improve Scores', desc: 'Raise HEDIS and quality performance' },
    { icon: Shield, title: 'Build Trust', desc: 'Confidence for payers, providers, and employers' },
  ];
...
const HowItWorksSection = () => {
  ...
  const steps = [
    { title: 'Unify', desc: 'Claims, EHR, labs, pharmacy, and files', icon: FileText },
    { title: 'Detect', desc: 'Surface quality gaps and unmet measures', icon: Eye },
    { title: 'Activate', desc: 'Trigger outreach, tasks, and reminders', icon: Zap },
    { title: 'Track', desc: 'Monitor closure and score improvement', icon: BarChart3 },
  ];
    ...
    <section ref={sectionRef} className="py-24 bg-gradient-to-b from-slate-50 to-white">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground">Continuous quality management, not annual audits</p>
        </div>
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <svg className="absolute -right-4 top-1/2 -translate-y-1/2 w-12 h-12 hidden lg:block opacity-30">
            ...
            />
            <polygon points="6,26 6,34 12,30" fill="currentColor" className="text-primary" />
          </svg>
...
const MeasuresSection = () => {
  ...
  const tiles = [
    { icon: Heart, title: 'Preventive Screenings', detail: 'Mammograms, colonoscopies, and wellness visits tracked and scheduled automatically' },
    { icon: Activity, title: 'Chronic Condition Mgmt', detail: 'Diabetes, hypertension, and heart disease measures with proactive intervention' },
    { icon: Pill, title: 'Medication Adherence', detail: 'Refill gaps detected and patient nudges triggered before issues arise' },
    { icon: Calendar, title: 'Care Follow-ups', detail: 'Post-discharge and specialist follow-up tracking with automated reminders' },
    { icon: Users, title: 'Population Health', detail: 'Aggregate quality metrics across patient populations and risk segments' },
    { icon: Settings, title: 'Custom Programs', detail: 'Configure custom measures for employer wellness or payer-specific requirements' },
  ];
...
const QualityInPracticeSection = () => {
  const results = [
    { before: 'Retrospective reporting', after: 'Real-time gap visibility', icon: Eye },
    { before: 'Manual outreach', after: 'Automated patient nudges', icon: Zap },
    { before: 'Static scores', after: 'Continuous improvement', icon: TrendingUp },
  ];
