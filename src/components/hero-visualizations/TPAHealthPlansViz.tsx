...
import { BarChart3, PieChart, LineChart, Activity, Shield, TrendingUp } from 'lucide-react'

const TPAHealthPlansViz = () => {
  const [activeMetric, setActiveMetric] = useState(0)
  
  const metrics = [
    { icon: LineChart, label: 'Utilization', trend: '+12%', color: '#8B5CF6' },
    { icon: PieChart, label: 'Cost Analysis', trend: '-8%', color: '#E94E87' },
    { icon: Activity, label: 'Engagement', trend: '+24%', color: '#06B6D4' },
    { icon: Shield, label: 'Risk Score', trend: 'Stable', color: '#10B981' },
  ]
  ...
  return (
    <div className='relative h-[450px] flex items-center justify-center'>
          ...
          <div className='relative space-y-4'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                ...
                <div className='text-xs text-muted-foreground'>Continuous insight</div>
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              <div className='flex items-center gap-1'>
                <TrendingUp className={`w-3 h-3 ${metric.trend.startsWith('+') ? 'text-green-500' : metric.trend.startsWith('-') ? 'text-red-500' : 'text-blue-500'}`} />
                <span className={`text-xs font-semibold ${metric.trend.startsWith('+') ? 'text-green-600' : metric.trend.startsWith('-') ? 'text-red-600' : 'text-blue-600'}`}>
                  {metric.trend}
                </span>
              </div>
            </div>
      ...
      <svg className='absolute inset-0 w-full h-full pointer-events-none'>
        <defs>
          <linearGradient id='tpaGrad' x1='0%' y1='0%' x2='100%' y2='0%'>
...
export default TPAHealthPlansViz
