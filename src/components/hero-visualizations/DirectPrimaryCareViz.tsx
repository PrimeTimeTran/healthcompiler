...
import { Users, Calendar, Heart, TrendingUp, Stethoscope, Clock } from 'lucide-react'

const DirectPrimaryCareViz = () => {
  const [activeMetric, setActiveMetric] = useState(0)
  
  const metrics = [
    { icon: Users, label: 'Members', value: '847 active', color: '#06B6D4' },
    { icon: Calendar, label: 'Visits', value: '124/week', color: '#8B5CF6' },
    { icon: Heart, label: 'Wellness', value: '92% engaged', color: '#E94E87' },
    { icon: Clock, label: 'Access', value: 'Same-day', color: '#10B981' },
  ]
    ...
    <div className='relative h-[450px] flex items-center justify-center'>
      ...
      <div className='relative'>
        ...
        <div className='relative bg-white rounded-2xl border border-cyan-500/20 shadow-2xl p-6 w-[180px]'>
          <div className='absolute inset-0 rounded-2xl bg-gradient-to-br from-cyan-500/5 to-violet-500/5' />
          ...
          <div className='relative space-y-4 text-center'>
            ...
            <div>
              <div className='text-sm font-semibold text-foreground'>DPC Practice</div>
              <div className='text-xs text-muted-foreground mt-1'>Membership insights</div>
            </div>
            
            <div className='pt-3 border-t border-border/50'>
              <div className='flex items-center justify-center gap-2'>
                <TrendingUp className='w-4 h-4 text-green-500' />
                <span className='text-xs text-green-600 font-medium'>Growth tracking</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <svg className='absolute inset-0 w-full h-full pointer-events-none'>
        <defs>
          <linearGradient id='dpcGrad' x1='0%' y1='0%' x2='100%' y2='100%'>
            <stop offset='0%' stopColor='#06B6D4' stopOpacity='0.3' />
            <stop offset='50%' stopColor='#8B5CF6' stopOpacity='0.5' />
            <stop offset='100%' stopColor='#E94E87' stopOpacity='0.3' />
          </linearGradient>
        </defs>
        <circle
          cx='50%'
...
export default DirectPrimaryCareViz
